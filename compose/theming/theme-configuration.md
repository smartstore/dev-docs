...
Variables are made available to other Sass files by including a virtual Sass file _/.app/themevars.scss_. This file translates each theme variable into a Sass variable. To do this, use the following `Include` in your `theme.scss` file. This should be the first file included in `theme.config` so that the variables are available in other included Sass files.
...
We have written a helper function in `ConfigureTheme.cshtml` for the output of the controls, with which it is possible to output the editor for a variable in one line:
